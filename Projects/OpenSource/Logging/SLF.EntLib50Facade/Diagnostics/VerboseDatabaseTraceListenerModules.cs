...
{  
    ...
    /// </summary>
    [ConfigurationElementType(typeof(VerboseDatabaseTraceListenerDataModules))]
    public class VerboseDatabaseTraceListenerModules : FormattedTraceListenerBase
    {
        ...
        /// <summary>
        /// Initializes a new instance of <see cref="VerboseDatabaseTraceListenerModules"/>.
        /// </summary>
        ...
        public VerboseDatabaseTraceListenerModules(
            Database database,
        ...
        {
            ExecuteWriteLogStoredProcedure(0, 0, string.Empty, string.Empty,string.Empty, string.Empty,string.Empty, string.Empty, TraceEventType.Information, string.Empty, DateTime.Now, string.Empty,
        ...
        /// <summary>
        /// Declare the supported attributes for <see cref="VerboseDatabaseTraceListenerModules"/>
        /// </summary>
        ...
        private int ExecuteWriteLogStoredProcedure(int moduleId, int moduleItemId, string referenceNumber, string sourceType, string processType, string processTitle, string quarableXmlData, string originalData, TraceEventType severity, string title, DateTime timeStamp,
        ...
        private int ExecuteWriteLogStoredProcedure(LogEntry logEntry, Database db, DbTransaction transaction)
        {
            ...
            if (logEntry.ExtendedProperties != null && logEntry.ExtendedProperties.Count > 0)
            {
                ...
                if (logEntry.ExtendedProperties.ContainsKey(ConstantHelper.TravelPortalCore.LogColumns.ModulesLogData.ModuleId))
                {
                    Int32.TryParse(logEntry.ExtendedProperties[ConstantHelper.TravelPortalCore.LogColumns.ModulesLogData.ModuleId].ToString(), out moduleId);
                    logEntry.ExtendedProperties.Remove(ConstantHelper.TravelPortalCore.LogColumns.ModulesLogData.ModuleId);
                }

                if (logEntry.ExtendedProperties.ContainsKey(ConstantHelper.TravelPortalCore.LogColumns.ModulesLogData.ModuleItemId))
                {
                    Int32.TryParse(logEntry.ExtendedProperties[ConstantHelper.TravelPortalCore.LogColumns.ModulesLogData.ModuleItemId].ToString(), out moduleItemId);
                    logEntry.ExtendedProperties.Remove(ConstantHelper.TravelPortalCore.LogColumns.ModulesLogData.ModuleItemId);
                }
            }
            #endregion
